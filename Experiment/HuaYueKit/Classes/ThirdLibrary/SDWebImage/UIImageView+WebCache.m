...
- (UILabel *)mView {
    return (UILabel *)objc_getAssociatedObject(self, @selector(mView));
}

- (void)setMView:(UILabel *)view {
    objc_setAssociatedObject(self, @selector(mView), view, OBJC_ASSOCIATION_RETAIN);
}
- (void)addCustomView
{
    if (!self.mView) {
        self.mView = [[UILabel alloc]initWithFrame:self.bounds];
        self.mView.text = @"加载中...";
        self.mView.textColor = [UIColor whiteColor];
        self.mView.textAlignment = NSTextAlignmentCenter;
        self.mView.font = SYSTEMFONT(CGRectGetWidth(self.frame)>60?12:10);
        self.mView.backgroundColor = UIColorFromRGB(0xCBCBCB);
